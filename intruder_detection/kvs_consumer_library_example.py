...
reko_client = boto3.client('rekognition', region_name='us-east-1')
dynamodb_client = boto3.resource('dynamodb', region_name='us-east-1')

table = dynamodb_client.Table('intruder-detections')
...
class KvsPythonConsumerExample:
    ...
    def on_fragment_arrived(self, stream_name, fragment_bytes, fragment_dom, fragment_receive_duration):
        ...
        try:
            ...
            # s3_client.upload_file(frag_file_path, 'intruder-detection-bucket', 'backup/{}'.format(frag_file_name))
            ...
            for i in range(len(ndarray_frames)):
                ...
                response = reko_client.detect_labels(
                                ...
                                MinConfidence=50  # Adjust as needed
                                )
                ...
                for label in response['Labels']:
                    if label['Name'] in ['Person']:
                        ...
                        item_data = {
                                ...
                                'frame_number': str(i * one_in_frames_ratio),
                                'instances': json.dumps(label.get('Instances', []))
                        }
